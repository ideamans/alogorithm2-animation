...
export const AnimatedIcon: React.FC<AnimatedIconProps> = ({
  ...
  const generateTriangles = (seed: string, animationMode: 'fly' | 'morph'): Triangle[] => {
    const fullSeed = markDefaults.seedPrefix + seed

    const pattern = Trianglify({
      width: size,
      height: size,
      variance: markDefaults.variance,
      cellSize: size * markDefaults.cellSizeRatio,
      seed: fullSeed,
    })
  ...
  const generateBlobPath = (seed: string): string => {
    ...
    const blobPath = Blobs2.svgPath({
      seed: fullSeed,
      extraPoints: markDefaults.extraPoints,
      randomness: markDefaults.randomness,
      size: 256,
    })
  ...
  useEffect(() => {
    const initializeMorph = async () => {
      ...
      const triangles = generateTriangles(initialSeed, mode)
  ...
  const startNewMorph = () => {
    ...
    const newTriangles = generateTriangles(newSeed, mode)
